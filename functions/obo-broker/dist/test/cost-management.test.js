...
import { actualCostRequest, buildActualCostSnapshot, createActualCostQuery } from '../src/cost-management.js';
...
test('uses ActualCost with a bounded custom daily query', () => {
    ...
    assert.deepEqual(request.dataset.grouping, [
        { type: 'Dimension', name: 'ResourceId' },
        { type: 'Dimension', name: 'ServiceName' },
    ]);
});
test('reports unavailable billing separately from telemetry', async () => {
    const port = { query: async () => { throw new Error('429'); } };
