...
async function handle(route, request, context, action) {
    let objectId;
    let applicationId;
    let dimensions;
    let status = 503;
...
app.http('exchange', {
    ...
    handler: (request, context) => handle('exchange', request, context, async (body, _objectId, assertion, expiresAt) => {
        ...
        if (target !== 'fabric' && target !== 'powerbi')
            throw new BrokerError(400, 'invalid_target');
        const token = await runtime().exchange(assertion, target, expiresAt);
...
app.http('tokenomics', {
    ...
    handler: (request, context) => handle('tokenomics_summary', request, context, async () => (runtime().tokenomics(tokenomicsWindow(request.query.get('days'))))),
});
