...
function aggregate(items) {
    ...
    for (const item of items)
        totals.set(item.name || 'Unspecified', (totals.get(item.name || 'Unspecified') ?? 0) + item.cost);
    return [...totals.entries()]
        .map(([name, cost]) => ({ name, cost: round(cost) }))
        .sort((left, right) => right.cost - left.cost);
}
function trackedResource(resources, resourceId) {
    const normalized = resourceId.toLowerCase();
    return resources.find(resource => resource.resourceId.toLowerCase() === normalized);
}
export function buildActualCostSnapshot(pages, config, generatedAt = new Date()) {
    ...
    const sharedPlatformCost = trackedRows
        ...
        .reduce((total, row) => total + row.cost, 0);
    const dates = normalized.map(row => row.date).filter((date) => date !== null);
...
function unavailableSnapshot(config, status) {
    return {
...
export function actualCostRequest(days, now = new Date()) {
...
function createCostManagementPort(config) {
    ...
    return {
        async query(scope, apiVersion, body) {
            ...
            if (!token?.token)
                throw new Error('cost_management_token_unavailable');
            const pages = [];
            let url = `https://management.azure.com${scope}/providers/Microsoft.CostManagement/query?api-version=${encodeURIComponent(apiVersion)}`;
            while (url) {
                let page;
                for (let attempt = 0; attempt < 3; attempt += 1) {
                    ...
                    if (!response.ok)
                        throw new Error(`cost_management_${response.status}`);
                    page = await response.json();
                    break;
                }
                if (!page)
                    throw new Error('cost_management_retry_exhausted');
                pages.push(page);
...
export function createActualCostQuery(config, client) {
    const queryClient = client ?? createCostManagementPort(config);
    return async (days) => {
        if (!config.actualCostEnabled)
            return unavailableSnapshot(config, 'disabled');
        try {
