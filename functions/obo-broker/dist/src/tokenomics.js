...
export const tokenomicsWindows = [1, 7, 30, 90];
function text(value) {
...
export function rowsFromTables(tables) {
    return tables.flatMap(table => table.rows.map(row => Object.fromEntries(table.columnDescriptors.map((column, index) => [column.name, row[index]]))));
}
export function tokenomicsWindow(value) {
    const days = Number(value ?? 30);
    if (!tokenomicsWindows.includes(days))
        throw new BrokerError(400, 'invalid_window');
    return days;
}
function rateFor(rateCard, model, at) {
...
function priceRows(rows, rateCard) {
    return rows.map(row => {
        ...
        if (!rate || totalTokens === 0)
            return { ...row, MarketCost: null, NegotiatedCost: null, PricedTokens: 0 };
...
export function buildTokenomicsDashboard(rows, config, days, queryStatus) {
...
export function createTokenomicsQuery(config, client) {
    ...
    return async (days) => {
        ...
        if (!tables.length)
            throw new BrokerError(503, 'tokenomics_unavailable');
